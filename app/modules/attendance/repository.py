...
class AttendanceRepository:
    ...
    async def get_by_lesson(self, session: AsyncSession, lesson_id: int) -> list[Attendance]:
        ...
        result = await session.scalars(
            select(Attendance).options(*self.related_options())
    ...
    async def get_valid_subscription(self, session: AsyncSession, student_id: int,
                                     branch_id: int, lesson_date: date) -> StudentSubscription | None:
