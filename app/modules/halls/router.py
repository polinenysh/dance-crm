...
async def get_halls(session: SessionDep, current_user: AdminOrOwnerDep,
                    branch_id: int | None = Query(default=None, gt=0)) -> list[HallResponse]:
...
async def create_hall(data: HallCreate, session: SessionDep,
                      current_user: AdminOrOwnerDep) -> HallResponse:
...
async def get_hall(hall_id: int, session: SessionDep,
                   current_user: AdminOrOwnerDep) -> HallResponse:
...
async def update_hall(hall_id: int, data: HallUpdate, session: SessionDep,
                      current_user: AdminOrOwnerDep) -> HallResponse:
